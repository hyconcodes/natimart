<x-layouts::app :title="__('Vendor Dashboard')">
    <div class="space-y-8">
        ...
        @if (auth()->user()->shop && !auth()->user()->shop->is_approved)
            <div
                class="bg-amber-50 border border-amber-200 text-amber-800 rounded-2xl p-4 flex items-start gap-4 shadow-sm">
                ...
                <div>
                    ...
                    <p class="text-sm font-medium text-amber-800/80">
                        Your store <strong>{{ auth()->user()->shop->name }}</strong> is currently under review by the
                        {{ ucfirst(auth()->user()->shop->state) }} State Coordinator. Your storefront
                        (<a href="http://{{ auth()->user()->shop->slug }}.{{ env('APP_DOMAIN', 'localhost') }}:8000"
                            class="underline hover:text-amber-900"
                            target="_blank">{{ auth()->user()->shop->slug }}.{{ env('APP_DOMAIN', 'localhost') }}</a>)
        ...
        @elseif(auth()->user()->shop && auth()->user()->shop->is_approved)
            <div
                class="bg-green-50 border border-green-200 text-green-800 rounded-2xl p-4 flex items-start gap-4 shadow-sm">
            ...
            <div
                ...
                <div>
                    ...
                    <div class="text-2xl font-black text-gray-900 dark:text-gray-100">
                        {{ auth()->user()->shop ? auth()->user()->shop->products()->count() : 0 }}</div>
                </div>
            </div>
        ...
        @if (auth()->user()->shop && !auth()->user()->shop->is_approved)
            <div class="space-y-6">
                <div
                    class="bg-brand-950 p-8 rounded-[2.5rem] relative overflow-hidden text-white border-2 border-brand-800 shadow-2xl">
                    <div class="absolute top-0 right-0 p-8 opacity-10">
                        <flux:icon name="shield-check" class="size-32" />
                    </div>
                    <div class="relative z-10">
                        <h2 class="text-3xl font-black italic mb-2">Verification Required</h2>
                        <p class="text-brand-300 max-w-lg mb-8 leading-relaxed">Your store is currently in "Draft" mode.
                            Submit your business credentials to get verified by your State Coordinator and start
                            selling.</p>

                        <div class="flex flex-col sm:flex-row items-center gap-6">
                            <flux:button
                                href="{{ route('vendor.verification.status', ['shop_slug' => auth()->user()->shop->slug]) }}"
                                variant="primary" icon="shield-check" wire:navigate>
                            ...
                            @if (auth()->user()->shop->verification)
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-32 h-2 bg-brand-900 rounded-full overflow-hidden border border-brand-800">
                                        @php
